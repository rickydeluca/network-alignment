...
# class Distill_ContrastiveLossWithAttention(nn.Module):
#     def __init__(self):
#         super(Distill_ContrastiveLossWithAttention, self).__init__()

#     def forward(self, pred_dsmat: Tensor, gt_perm: Tensor,
#                 src_ns: Tensor, tgt_ns: Tensor, beta_value,
#                 momentum=False) -> Tensor:
        
#         batch_num = pred_dsmat.shape[0]

#         # Clamp the predicted alignment matrix between 0 and 1
...
#         # For each element keep only the alignment score with
#         # the corresponding groundtruth (S_i,pi(i))
...
#         def calculateLoss(pred_dsmat, gt_perm, gt_predicted_values, row_gt_values, 
#                           column_gt_values, attention_src_negatives_selected,
#                           attention_tgt_negatives_selected, all_zeros_tensor, b):
...
#         n_sum = torch.zeros_like(loss)
#         for b in range(batch_num):
#             loss += calculateLoss(pred_dsmat[b, :src_ns[b], :tgt_ns[b]], gt_perm[b, :src_ns[b], :tgt_ns[b]], 
#                                   gt_predicted_values[b, :src_ns[b], :tgt_ns[b]], row_gt_values[b, :src_ns[b], :tgt_ns[b]],
#                                   column_gt_values[b, :src_ns[b], :tgt_ns[b]], attention_src_negatives_selected[b, :src_ns[b], :tgt_ns[b]],
#                                   attention_tgt_negatives_selected[b, :src_ns[b], :tgt_ns[b]], all_zeros_tensor[b, :src_ns[b], :tgt_ns[b]],
#                                   b)
