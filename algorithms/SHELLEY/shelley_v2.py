"""
Train on 20% of the nodes, eval on the remaining 80%.
"""
...
def assemble_model(head: str, backbone: str, cfg: dict, device: torch.device = 'cpu'):
    ...
    if backbone == 'gin':
        ...
        raise ValueError(f"[SHELLEY] Invalid backbone: {backbone}.")
    ...
    if head == 'common':
        ...
        raise ValueError(f"[SHELLEY] Invalid head: {head}.")
...
class SHELLEY(NetworkAlignmentModel):
    def __init__(self, source_dataset, target_dataset, args):
        ...
        self.cfg.DATA.SELF_SUPERVISED = args.self_supervised
        self.cfg.DATA.SPLIT_RATIO = args.split_ratio
        self.cfg.DATA.SIZE = args.size
    ...
    def align(self):
        ...
        full_dataset = GMSemiSyntheticDataset(root_dir=self.cfg.DATA.ROOT_DIR,
                                              ...
                                              split_ratio=self.cfg.DATA.SPLIT_RATIO)  # 20% training / 80% test
        

        # Train / test split
        train, test = random_split(full_dataset, (0.7, 0.3))
        dataset = {'train': train, 'test': test}
        
        self.dataloader = {x: get_dataloader(
            dataset[x],
            shuffle=True,
            fix_seed=(x != 'train'),
        ...
        for inputs in self.dataloader['train']:
            ...
            with torch.set_grad_enabled(True):
                ...
                if 'common' in self.cfg.HEAD.NAME: # COMMON use the iter number to control the warmup temperature
                    outputs = self.model(inputs, training=True, use_train_subset=True, iter_num=iter_num, epoch=epoch)
                else:
                ...
                # Compute accuracy
                acc = matching_accuracy(outputs['train_perm_mat'], outputs['train_gt_perm_mat'], outputs['train_ns'], idx=0)[0]
    ...
    def eval_model(self, mode='val'):
        ...
        if mode == 'val':
            # Use the same dataset used in trainig,
            # but evaluate on the remainig nodes.
        ...
        with torch.no_grad():
            for eval_inputs in dataloader:
                ...
                # Forward step
                eval_outputs = self.model(eval_inputs, training=False, use_test_subset=True)

                # Compute accuracy
                eval_acc += matching_accuracy(eval_outputs['test_perm_mat'], eval_outputs['test_gt_perm_mat'], eval_outputs['test_ns'], idx=0)[0].item()
