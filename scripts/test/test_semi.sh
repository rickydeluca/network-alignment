...
# EDGE_REMOVAL=("0" "01" "05" "1" "2" "3" "5" "7" "9")
...
for ER in "${EDGE_REMOVAL[@]}"; do
    PREFIX="REGAL-d${ER}-seed1"  # Target network directory
    ...
    for ALGO_CONFIG in "${ALGORITHMS[@]}"; do
        ...
        # Split the algorithm configuration to get 
        # the algorithm name and its arguments
        ...
        for ((i=1; i<=$RUNS; i++)); do
            ...
            if [ "$DATA" == "edi3" ] || [ "$DATA" == "ppi" ]; then
                ./scripts/dataset/gen_semi.sh $DATA $TR $ER
            fi
        ...
        MEAN_ACCURACY=$(echo "scale=4; $TOTAL_ACCURACY / $RUNS" | bc)
        MEAN_EXECUTION_TIME=$(echo "scale=4; $TOTAL_EXECUTION_TIME / $RUNS" | bc)
        RESULTS_DICT["$ALGORITHM_NAME,0.$ER,$TR"]=$MEAN_ACCURACY,$MEAN_EXECUTION_TIME

    done
done

# Write the results to a CSV file
RESULTS_CSV="results/${ALG}_${DATA}_s${TR}_n${RUNS}.csv"
echo "algorithm,edge_removal_ratio,train_ratio,mean_accuracy,mean_execution_time" > $RESULTS_CSV
